...
async function serverStudySearch(courses: string[]): Promise<StudyGroup[]> {
	...
	return Promise.all(
		snapshot.docs.map(
			async (doc: any): Promise<StudyGroup> =>
...
async function serverUserStudySearch(user: string): Promise<StudyGroup[]> {
	const snapshot = await db
		.collection("groups")
		.where("end", ">", Timestamp.now())
		.where("buddies", "array-contains-any", [user])
		.get();
	return Promise.all(
		snapshot.docs.map(
			async (doc: any): Promise<StudyGroup> =>
...
async function createStudyGroupType(
	docId: string,
	data: any
): Promise<StudyGroup> {
	const buddies = await Promise.all(
		data.buddies.map(async (buddy: string) => {
...
async function serverStudyBook(
	user: string,
	booking: StudyBooking
): Promise<WriteResult> {
	return await db
		.collection("groups")
		.doc()
